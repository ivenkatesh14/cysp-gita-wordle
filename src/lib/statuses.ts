...
export const generateRedundancyWarning = (current_guess: string, past_guesses: string[]): string => {
  ...
  for (const past_guess of past_guesses) {
    ...
    for (var i = WORDLEN-1; i>=0; i--) {
        if (!denormsol.includes(splitpast[i][0]) && splitguess.map(l => l[0]).includes(splitpast[i][0])) {
            ret = "You know already that \"" + splitpast[i][0] + "\" isn't in the word.";
        }
        if (denormsol.includes(splitpast[i][0]) && answer[i][0] !== splitpast[i][0] && splitguess[i][0] === splitpast[i][0]) {
        ...
        if (!ret && answer[i][0] === splitpast[i][0] && splitguess[i][0] !== answer[i][0]){
            ret = "Letter " + (i+1) + " should be some form of \"" + answer[i][0] + "\"";
        }
        if (!ret && denormsol.includes(splitpast[i][0]) && !splitguess.map(l => l[0]).includes(splitpast[i][0])) {
            ret = "The secret word has some version of \"" + splitpast[i][0] + "\" in it.";
        }
    }
  }
  if (!ret) return '';
  return 'Warning! ' + ret + ' Press ⏎ again to submit this guess anyway. Hit backspace to delete.';
}
